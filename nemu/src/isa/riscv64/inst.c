...
#define INSTPAT_MATCH(s, name, type, ... /* body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 100 ????? 11000 11", blt    , B, if (getSigned(src1) < getSigned(src2)) s->dnpc = s->pc + dest);
  INSTPAT("??????? ????? ????? 101 ????? 11000 11", beq    , B, if (getSigned(src1) >= getSigned(src2)) s->dnpc = s->pc + dest);
  // load
  ...
  // operation
  INSTPAT("??????? ????? ????? 000 ????? 01110 11", addw   , R, R(dest) = getWord(src1 + src2));
  INSTPAT("0000000 ????? ????? 000 ????? 01100 11", add    , R, R(dest) = src1 + src2);
  INSTPAT("0100000 ????? ????? 000 ????? 01100 11", sub    , R, R(dest) = src1 - src2);
  INSTPAT("??????? ????? ????? 000 ????? 00100 11", addi   , I, R(dest) = src1 + src2);
  INSTPAT("??????? ????? ????? 000 ????? 00110 11", addiw  , I, R(dest) = getWord(src1 + src2));
  INSTPAT("??????? ????? ????? 011 ????? 00100 11", sltiu  , I, R(dest) = src1 < src2);
  // RV32M standard extension
  INSTPAT("0000001 ????? ????? 110 ????? 01110 11", remw   , R, R(dest) = getWord(getSigned(src1) % src2));
  /* end */  
