...
#define INSTPAT_MATCH(s, name, type, ... /* body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 000 ????? 00000 11", lbu    , I, R(dest) = Mr(src1 + src2, 1));
  INSTPAT("??????? ????? ????? 001 ????? 00000 11", lhu    , I, R(dest) = Mr(src1 + src2, 2));
  // store 
  INSTPAT("??????? ????? ????? 000 ????? 01000 11", sb     , S, Mw(src1 + dest, 1, src2));
  INSTPAT("??????? ????? ????? 001 ????? 01000 11", sh     , S, Mw(src1 + dest, 2, src2));
  INSTPAT("??????? ????? ????? 010 ????? 01000 11", sw     , S, Mw(src1 + dest, 4, src2));
  // operation
  INSTPAT("??????? ????? ????? 000 ????? 01110 11", addw   , R, R(dest) = WO(src1 + src2));
  ...
  INSTPAT("0000000 ????? ????? 101 ????? 01100 11", srl    , R, R(dest) = SI(src1) >> src2);
  ...
  INSTPAT("??????? ????? ????? 000 ????? 00110 11", addiw  , I, R(dest) = WO(src1 + src2));
  INSTPAT("??????? ????? ????? 011 ????? 00100 11", sltiu  , I, R(dest) = src1 < src2);
  INSTPAT("0000000 ????? ????? 001 ????? 00100 11", slli   , I, R(dest) = src1 << src2);
  INSTPAT("0000000 ????? ????? 101 ????? 00100 11", srli   , I, R(dest) = SI(src1) >> src2);
  INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai   , I, R(dest) = src1 >> src2);
  // RV32M standard extension
  INSTPAT("0000001 ????? ????? 110 ????? 01110 11", remw   , R, R(dest) = WO(src1) % WO(src2));
  /* end */  
